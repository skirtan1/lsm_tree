...
#[test]
fn test_block_iterator_first_seek_then_next() {
    ...
    assert_eq!(
        ...
        key_of(1).for_testing_key_ref(),
        "expected key: {:?}, actual key: {:?}",
        as_bytes(key_of(1).for_testing_key_ref()),
...
#[test]
fn test_block_seek_key_once() {
    let block = Arc::new(generate_block());
    let iter = BlockIterator::create_and_seek_to_key(block, key_of(0).as_key_slice());
    let key = iter.key();
    let value = iter.value();
    assert_eq!(
        key.for_testing_key_ref(),
